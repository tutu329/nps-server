...
def get_db_user_obj(in_username):
    user_obj = User.objects.get(username=in_username)
    return user_obj
...
class UserProfile(models.Model):
    ...
    nickname = models.CharField(max_length=50, default='普通用户')       # User昵称
    gender = models.CharField(max_length=10, default='男')              # 性别
    user_level = models.IntegerField(default=1)                         # User等级
    vip_expired = models.BooleanField(default=True)                     # VIP是否过期
    vip_start_time = models.DateTimeField(blank=True, null=True)        # VIP开始时间
    vip_days = models.IntegerField(default=0)                           # VIP剩余天数

    gpt4_invoke_num_left_today = models.IntegerField(default=0)         # User今天剩下的GPT4调用次数
...
class Role(models.Model):
    ...
    temperature = models.FloatField(default=0.8)                        # GPT的严谨程度(越小越稳定、越大越多变)
    presence_penalty = models.FloatField(default=1.0)                   # GPT的出现惩罚(越小主题越少)
    frequency_penalty = models.FloatField(default=1.0)                  # GPT的重复惩罚(越小重复越多)
